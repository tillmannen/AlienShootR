...
public class OldGUIExamplesCS : MonoBehaviour {
	...
	void OnGUI () {
		...
		if(GUI.Button(buttonRect1.rect, "Bounce")){
			
			//LeanTween.move( buttonRect1, new Vector2(buttonRect1.rect.x, buttonRect1.rect.y-15) /* * 1.2f*/, 0.00f ); //.setEase( LeanTweenType.easeOutQuad );
			LeanTween.move( buttonRect1, new Vector2(buttonRect1.rect.x, buttonRect1.rect.y), 1.5f ).setEase(LeanTweenType.easeOutBounce);
		}
